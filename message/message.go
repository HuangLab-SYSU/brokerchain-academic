...
func MergeMessage2(addr string, content []byte) []byte {
	if len(addr) > prefixMSGtypeLen2{
		return []byte{}
	}
	b := make([]byte, prefixMSGtypeLen2)
	for i, v := range []byte(addr) {
		b[i] = v
	}
	merge := append(b, content...)
	return merge
...
func SplitMessage2(message []byte) (string, []byte) {
	msgTypeBytes := message[:prefixMSGtypeLen2]
	msgType_pruned := make([]byte, 0)
	for _, v := range msgTypeBytes {
		if v != byte(0) {
			msgType_pruned = append(msgType_pruned, v)
