...
func main() {
	...
	if global.Version != string(version) {
		fmt.Println()
		fmt.Println("=========================================")
	...
	go func() {
		for {
			time.Sleep(10 * time.Second)
			version, err := Get("getversion", []byte{})
			if err != nil {
				continue
			}
			if global.Version != string(version) {
				fmt.Println()
				fmt.Println("======================================")
				fmt.Println("Client version too old. Please visit https://github.com/HuangLab-SYSU/brokerchain-academic and update your client.")
				fmt.Println("======================================")
				fmt.Println()
	...
	case "1":
		...
		// 调用API获取公私钥
		//publicKey, privateKey, err = fetchKeysFromAPI()
	...
	//r.LoadHTMLFiles(htmlFiles.ReadFile("index.html"))
	r.LoadHTMLGlob("html/*")
