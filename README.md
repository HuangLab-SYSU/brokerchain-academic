...
About 20% of the supply is reserved for community-building initiatives and incentivizing early developers, ensuring sustainable ecosystem growth. To promote long-term value alignment, the remaining tokens enter circulation via a decaying emission mechanism, reducing block rewards by half every four years.
...
- **Apply for Faucet Tokens dedicated for Developers**: Through the faucet service, you can request tokens specifically designed for developers.
...
1. Download the release compressed file of the BrokerChain (academic) executable file and extract it to any folder. Be careful not to double-click the brokerchain.academic_macos file; otherwise, the system will prompt 'unable to open'.
...
5. To access the browser wallet page, you need to manually copy the wallet's browser address.

During the process of starting and running brokerchain_academic_macos, you may encounter some "resistance," which is explained below.
...
- Due to deploying IP list seed servers abroad, issues such as an inability to connect to the servers or network connection timeouts may occur. At this point, you need to close the current program and try rerunning it multiple times until you connect to the server.
...
When users encounter the above situation or similar situations, they can restart the client program without worrying about losing their account balance. Therefore, we recommend that users ensure that their machine runs well and the network is unobstructed when using Miner features. It is best to keep running the client program continuously; otherwise, it will affect the benefits of participating in consensus.
