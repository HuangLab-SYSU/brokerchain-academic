...
func (cphm *CLPAPbftInsideExtraHandleMod) accountTransfer_do2(atm *message.M) {
	...
	bh := &core.BlockHeader{
		ParentBlockHash: cphm.pbftNode.CurChain.CurrentBlock.Hash,
		Number:          cphm.pbftNode.CurChain.CurrentBlock.Header.Number + 1,
		Time:            time.Time{},
	}

	rt := cphm.pbftNode.CurChain.CurrentBlock.Header.StateRoot
	st, _ := trie.New(trie.TrieID(common.BytesToHash(cphm.pbftNode.CurChain.CurrentBlock.Header.StateRoot)), cphm.pbftNode.CurChain.Triedb)
	infos := atm.Infos
	for _, info := range infos {
		addr := info.Addr
		if info.ShardId != cphm.pbftNode.ShardID{
	...
	if ns != nil {
		err := cphm.pbftNode.CurChain.Triedb.Update(trie.NewWithNodeSet(ns))
		if err != nil {
			fmt.Println(err)
		}
		err = cphm.pbftNode.CurChain.Triedb.Commit(rrt, false)
		if err != nil {
			log.Panic(err)
		}
		rt = rrt.Bytes()
	...
	bh.StateRoot = rt
	bh.TxRoot = chain.GetTxTreeRoot(emptyTxs)
	bh.Bloom = *chain.GetBloomFilter(emptyTxs)
	bh.Miner = 0
	b := core.NewBlock(bh, emptyTxs)
	b.Hash = b.Header.Hash()

	cphm.pbftNode.CurChain.CurrentBlock = b
	cphm.pbftNode.CurChain.Storage.AddBlock(b)

	cphm.cdm.ModifiedMap2 = nil
